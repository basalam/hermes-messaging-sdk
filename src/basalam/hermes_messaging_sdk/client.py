"""
Hermes API Client - Main client for triggering workflows
"""
...
class HermesClient:
    """
    Lightweight client for triggering Hermes workflows.
    ...
    def __init__(
        ...
        # Create httpx client with persistent connection
    ...
    def _make_request(self, url: str, payload: Dict[str, Any]) -> Dict[str, Any]:
        """
        Make an HTTP POST request to the Hermes API with retry logic.
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                if response.status_code >= 400:
                    should_retry = self._handle_error_response(response, url, attempt)
                    if should_retry:
    ...
    def _handle_error_response(self, response: httpx.Response, url: str, attempt: int) -> bool:
        """
